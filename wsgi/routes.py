        ...
        for row in rows:
            ...
            if rowCntry in cntRes:
                if row[is_parked] != None:
                    if row[is_parked]:
                        if row[result] == "ok":
                            cntRes[rowCntry]["Parked domain"] = row[count]
                    else:
                        cntRes[rowCntry][row[result]] = row[count]
            else:
                if row[is_parked] != None:
                    if row[is_parked]:
                        if row[result] == "ok":
                            cntRes[rowCntry] = {"Country" : rowCntry,
                                                "Parked domain":row[count]}
                    else:
                        cntRes[rowCntry] = {"Country" : rowCntry,
        ...
        for row in rows:
            ...
            if rowCntry in cntRes:
                if row[is_parked] != None:
                    if row[is_parked]:
                        if row[result] == "ok":
                            cntRes[rowCntry].append({"name":"Parked domain",
                                                        "value":row[count]})
                    else:
                ...
                if row[is_parked] != None:
                    if row[is_parked]:
                        if row[result] == "ok":
                            cntRes[rowCntry] = [{"name":"Parked domain",
                                                        "value":row[count]}]
                    else:
    ...
    def formatRes_all_b(rows, resList):
        ...
        for row in rows:
            ...
            if row[is_parked] != None:
                if row[is_parked]:
                    if row[result] == "ok":
                        cntRes.append({"name":"Parked domain",
                                        "value":int(row[count])})
                else:
        ...
        for row in rows:
            ...
            if rowSrc in cntRes:
                if row[is_parked] != None:
                    if row[is_parked]:
                        if row[result] == "ok":
                            cntRes[rowSrc]["Parked domain"] = row[count]
                    else:
                        cntRes[rowSrc][row[result]] = row[count]
            else:
                if row[is_parked] != None:
                    if row[is_parked]:
                        if row[result] == "ok":
        ...
        for row in rows:
            ...
            if rowSrc in cntRes:
                if row[is_parked] != None:
                    if row[is_parked]:
                        if row[result] == "ok":
                            cntRes[rowSrc].append({"name":"Parked domain",
                                                        "value":row[count]})
                    else:
                ...
                if row[is_parked] != None:
                    if row[is_parked]:
                        if row[result] == "ok":
                            cntRes[rowSrc] = [{"name":"Parked domain",
                                                        "value":row[count]}]
                    else:
...
def overall_data():
    ...
    query = text("""
            WITH count_res AS (
                    ...
                    SELECT result, is_parked, avg(count) as "average_number", row_number() OVER (ORDER BY result, is_parked) AS "id"
                    FROM capture_counts
                    GROUP BY result, is_parked
                    ORDER BY result
                ), country_res AS (
            ...
            SELECT a.result, a.is_parked, a.average_number, c.country, 
                    c.c_perc, c.rank as c_rank, s.source, s.s_perc, 
                    s.rank as s_rank
        ...
        for row in result:
            if row["result"] != None:
                if row["is_parked"] == None:
                    ...
                    if row["result"] == "ok":
                        avgRes.append({"name":"Parked domain",
                                "value":float(round(row["average_number"],2))})
                        avgLabels.append("Parked domain")
                else:
